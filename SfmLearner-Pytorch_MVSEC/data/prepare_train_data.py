...
parser.add_argument("--dataset-format", type=str, default='kitti', choices=["kitti_raw", "kitti_odometry", "cityscapes", "mvsec_data"]) #Emmanuel code
...
def dump_example(args, scene):
    ...
    for scene_data in scene_list:
        ...
        for sample in data_loader.get_scene_imgs(scene_data):
            ...
            #dump_img_file = dump_dir/'{}.png'.format(frame_nb) #Emmanuel Code
            dump_img_file.parent.makedirs_p()
            #########Emmanuel Code##########
            #temp_img_file = dump_dir/'{}.png'.format(frame_nb)
            #im = Image.open(temp_img_file)
...
def main():
   ...
   #####################Emmanuel code################################################
    if args.dataset_format == 'mvsec_data':
        from kitti_raw_loader import KittiRawLoader
        data_loader = KittiRawLoader(args.dataset_dir,
                                     static_frames_file=args.static_frames,
                                     img_height=args.height,
