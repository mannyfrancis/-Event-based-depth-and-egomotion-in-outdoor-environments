...
def main():
    ...
    for topic, msg, t in bag.read_messages(
        ...
        if 'image' in topic:
            ...
            image = np.frombuffer(msg.data, dtype=np.uint8) #.reshape(image_data.height, image_data.width, -1) #Emmanuel Code
            #image = np.frombuffer(image_data.data, dtype=np.uint8) #.reshape(image_data.height, image_data.width, -1) #Emmanuel Code
